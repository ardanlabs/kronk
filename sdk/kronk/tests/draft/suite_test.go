...
func TestSuite(t *testing.T) {
	testlib.WithModel(t, testlib.CfgClassicDraftChat(), func(t *testing.T, krn *kronk.Kronk) {
		...
		t.Run("DraftAcceptance", func(t *testing.T) {
			...
			// It reproduced on ROCm 7.2.x in runs 33928022827 and 33929435557,
			// deterministically: draft=28 accepted=0 on every request. The
			// fleet is now on ROCm 10.0 with gfx1151 code objects, so the skip
			// comes off to find out whether that still holds.
