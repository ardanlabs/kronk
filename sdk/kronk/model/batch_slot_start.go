...
func (e *batchEngine) startSlot(s *slot, job *chatJob, buf []byte) {
	...
	// Prime the parser and slot to start in reasoning mode so generated tokens
	// are correctly classified until the corresponding close marker. Setting
	// reasonFlag ensures grammar sampling is skipped during the thinking phase.
	//
	...
	prompt := strings.TrimRight(job.prompt, " \t\r\n")
	if (strings.HasSuffix(prompt, "<think>") || strings.HasSuffix(prompt, "<|open|>think<|sep|>")) && job.params.Grammar == "" {
		// Drive the state machine into reasoning mode by feeding the same
		// marker the model would have emitted. Kimi K3 uses an XTML opener;
		// the other reasoning parsers use <think>.
		marker := "<think>"
		if strings.HasSuffix(prompt, "<|open|>think<|sep|>") {
