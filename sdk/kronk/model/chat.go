...
func (m *Model) sendChatError(ctx context.Context, ch chan<- ChatResponse, id string, err error) {
	...
	select {
	case ch <- ChatResponseErr(id, ObjectChatUnknown, m.responseModelID(), 0, err, Usage{}):
		return
	default:
	}
	...
	case <-ctx.Done():
		select {
		case ch <- ChatResponseErr(id, ObjectChatUnknown, m.responseModelID(), 0, ctx.Err(), Usage{}):
		default:
		}
