...
func RecurrentStateCopies(cfg Config, embeddedMTP bool) int64 {
	...
	if embeddedMTP {
		if mode != SpeculationClassic && speculative.Available() {
	...
	if mode != SpeculationClassic && cfg.MTPDrafterFile != "" && speculative.Available() {
		return int64(1 + mtpNDraft(cfg))
	}
...
// MUST be done here:
//
//  1. speculative.SetEmbeddingsNextN enables NextN hidden state extraction:
...
func loadDraftModelMTP(ctx context.Context, log applog.Logger, targetCtx llama.Context, targetModel llama.Model, targetCtxParams llama.ContextParams, nDraft int) (*mtpDrafter, error) {
	...
	speculative.SetEmbeddingsNextN(targetCtx, true, false)
	speculative.SetEmbeddingsNextN(lctx, true, true)
...
//  2. params.CtxType = MTP and speculative.SetEmbeddingsNextN enable NextN
...
// llama_set_embeddings_nextn(target, true, false) before the next
...
func selectAndLoadDraft(ctx context.Context, log applog.Logger, cfg Config, targetCtx llama.Context, targetModel llama.Model, targetCtxParams llama.ContextParams, plan speculationPlan) (drafter, error) {
	...
	case speculationSourceMTPCompanion:
		if !speculative.Available() {
			const reason = "MTPDrafterFile is a gemma4-assistant MTP head but the loaded llama library does not export the NextN hidden state APIs (llama_set_embeddings_nextn / llama_get_embeddings_nextn / _ith). MTP speculative decoding is DISABLED for this model. Install a libllama build that exports those symbols."
	...
	// loaded llama library does not export the NextN hidden state APIs
	// MTP needs. Without those, the MTP head would predict blind.
	...
	//
	// These symbols live in the llama.cpp staging header llama-ext.h, so
	// an older or differently built libllama can lack them. The yzma
	// exp/speculative package reports what it found.
	if !speculative.Available() {
		const reason = "target GGUF declares MTP (nextn_predict_layers>0) but the loaded llama library does not export the NextN hidden state APIs (llama_set_embeddings_nextn / llama_get_embeddings_nextn / _ith). MTP speculative decoding is DISABLED for this model. Install a libllama build that exports those symbols."
