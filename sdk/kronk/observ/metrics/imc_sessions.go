...
func newIMCSessionsCollector() *imcSessionsCollector {
	...
	c := imcSessionsCollector{
		...
		inputMessages: prometheus.NewDesc(
			"imc_session_latest_request_messages",
			"Number of messages in the latest completed request served by the IMC cache entry.",
			labels,
			nil,
		),
		inputTokens: prometheus.NewDesc(
			"imc_session_latest_request_input_tokens",
			"Complete input tokens in the latest completed request, including the generation-template tail.",
			labels,
			nil,
		),
		outputTokens: prometheus.NewDesc(
			"imc_session_latest_request_output_tokens",
			"Reasoning and completion tokens generated by the latest completed request.",
			labels,
			nil,
		),
		requestContext: prometheus.NewDesc(
			"imc_session_latest_request_context_tokens",
			"Total context reached by the latest completed request, including input and generated output tokens.",
			labels,
			nil,
		),
		peakContext: prometheus.NewDesc(
			"imc_session_peak_context_tokens",
			"Largest live context observed for the IMC cache entry, including generated output, in tokens.",
			labels,
			nil,
		),
		peakUsedPercent: prometheus.NewDesc(
			"imc_session_peak_used_percent",
			"Percentage of the configured context window represented by the peak live context.",
			labels,
			nil,
		),
...
func (c *imcSessionsCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, session := range provider() {
		...
		c.collect(ch, c.requestContext, float64(session.InputTokens+session.OutputTokens), modelID, entry)
		c.collect(ch, c.peakContext, float64(session.PeakContext), modelID, entry)
		...
		peakUsedPercent := 0.0
		if session.Window > 0 {
			peakUsedPercent = float64(session.PeakContext) / float64(session.Window) * 100
		}
		c.collect(ch, c.peakUsedPercent, peakUsedPercent, modelID, entry)
