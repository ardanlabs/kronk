...
// Path returns the directory the runtime should load stable-diffusion.cpp
// libraries from. Resolution mirrors the rules used by New (see
// WithLibPath):
//
//  1. An explicit override or KRONK_MALINA_LIB_PATH that already
...
func Path(override string) string {
	if override == "" {
		override = os.Getenv("KRONK_MALINA_LIB_PATH")
		if override == "" {
			override = os.Getenv("MALINA_LIB")
		}
	}
	...
	if err != nil {
		...
		if v := os.Getenv("KRONK_MALINA_LIB_PATH"); v != "" {
			return v
		}
