...
// physical memory pool, such as Apple Silicon or an integrated Vulkan GPU.
// When true, the manager tracks only the system-RAM pool and never adds a
// separate GPU device to its budget; this prevents double-counting the same
// physical bytes against two budgets. The flag is forwarded to Usage so
// observability code can label the single pool appropriately.
...
// manager will hand out. Defaults to DefaultBudgetPercent (90) when zero.
//
...
// shared GPU/CPU memory pool and only the RAM budget/used fields are
// populated. The Devices slice will be empty.
type Usage struct {
	BudgetPercent int
	HeadroomBytes int64
	Devices       []DeviceUsage
