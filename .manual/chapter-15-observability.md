...
| IMC | `imc_session_state`, `imc_session_context_tokens`, `imc_session_fallback_context_tokens`, `imc_session_latest_request_context_tokens`, `imc_session_peak_context_tokens`, `imc_snapshot_skipped_total`, `imc_pure_hit_stale_session_total` |
...
values in an **IMC Sessions** table.

| Metric | Meaning |
| ------ | ------- |
| `imc_session_state` | One-hot entry state. The `state` label is `active`, `idle`, or `empty`. |
| `imc_session_messages` | Complete messages represented by Current. |
| `imc_session_context_tokens` | Physical token context represented by Current. |
| `imc_session_allocated_tokens` | Largest physical token context retained by Current's backing allocation. |
| `imc_session_fallback_context_tokens` | Physical token context represented by Fallback; zero when absent. |
| `imc_session_fallback_allocated_tokens` | Largest physical token context retained by Fallback's backing allocation; zero when absent. |
| `imc_session_fallback_kind` | One-hot Fallback state. The bounded `kind` label is `user` or `calculating`; the series is absent when no Fallback exists. |
| `imc_session_fallback_updates` | Number of Fallback installations or advances for this bounded entry. This is a gauge because it resets when the entry is recycled. |
| `imc_session_latest_request_messages` | Messages in the latest completed request tracked by the entry. |
| `imc_session_latest_request_input_tokens` | Complete input tokens in that request, including the generation-template tail. |
| `imc_session_latest_request_output_tokens` | Reasoning and completion tokens generated by that request. |
| `imc_session_latest_request_context_tokens` | Latest request input plus generated output tokens. |
| `imc_session_peak_context_tokens` | Largest live context observed across the entry's requests, including generated output. |
| `imc_session_window_tokens` | Configured context window. |
| `imc_session_used_percent` | Current context divided by the configured window. |
| `imc_session_peak_used_percent` | Peak context divided by the configured window. |
| `imc_session_has_media` | `1` when the retained Current snapshot includes media; otherwise `0`. |
| `imc_session_last_used_timestamp_seconds` | Current eviction/LRU Unix timestamp; zero when never used. |

`calculating` is transient. It means Kronk is rebuilding Current from the
selected Fallback or has not yet published a verified user-boundary fallback.
The Current values can therefore move before the latest-request values update:
the former are published during prompt preparation, while the latter describe
the most recently completed request. Peak context is an entry lifetime high
water mark and can describe an earlier request.
