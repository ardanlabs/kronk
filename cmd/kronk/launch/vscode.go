...
func printVSCodeTip(defaultModel, baseURL string) {
	...
	fmt.Fprintf(os.Stderr, "     Command Palette), then choose \"OpenAI Compatible\" / \"Custom Endpoint\".\n")
	fmt.Fprintf(os.Stderr, "  3. Enter these values when prompted:\n")
	fmt.Fprintf(os.Stderr, "       Group/Provider name : %s\n", vscodeProviderName)
	fmt.Fprintf(os.Stderr, "       Base URL            : %s\n", baseURL)
	fmt.Fprintf(os.Stderr, "       API key             : %s\n", vscodeTipAPIKey())
	fmt.Fprintf(os.Stderr, "       Model id            : %s\n", defaultModel)
	fmt.Fprintf(os.Stderr, "  4. Pick the \"%s\" model in the model picker and start chatting.\n\n", defaultModel)
