...
export const PARAM_TOOLTIPS = {
  ...
  imcContext: 'Current rolling snapshot size. For text-only sessions this is the number of reusable token positions; media may expand one logical item into multiple physical KV-cache positions.',
  imcAllocated: 'Largest physical context retained by the rolling snapshot. It does not include the user-turn checkpoint.',
  imcCheckpointContext: 'Physical context retained at the previous complete user-turn boundary. Kronk can restore this checkpoint when later template output no longer matches the rolling snapshot.',
  imcCheckpointAllocated: 'Largest physical context retained by the user-turn checkpoint backing allocation.',
  imcTotalAllocated: 'High-water context allocated to this session, including generated output. It does not decrease when the session is reused with a smaller context. Internal fallback storage is not included.',
  imcPeakContext: 'Largest live context observed for this session, including cached or decoded prompt positions and generated output.',
  imcMessages: 'Number of complete messages represented by the latest rolling snapshot. Kronk normally restores this state when it prefixes the next prompt.',
  imcInputMessages: 'Number of messages in the latest completed request. The rolling snapshot is published during prompt setup, so it can temporarily contain more messages while a newer request is generating.',
  imcInputTokens: 'Complete input tokens in the latest completed request, including the stable conversation render and generation-template tail. Rolling tokens can temporarily be larger while a newer request is generating.',
  imcOutputTokens: 'Reasoning and completion tokens generated by the latest completed request.',
  imcFallbackMessages: 'Complete message count represented by the alternate fallback checkpoint. A dash means the checkpoint ends at an exact token boundary that cannot be mapped safely to a message count. This is not the message count reused by the latest request.',
  imcFallbackTokens: 'Tokens in the alternate fallback checkpoint. Kronk restores the longest compatible rolling or fallback state when planning a request. This is not the token count reused by the latest request.',
  imcRequestTotal: 'Latest completed request input tokens plus generated output tokens.',
  imcRequestUtilization: 'Latest completed request input plus output tokens divided by the configured context window.',
  imcContextWindow: 'Maximum configured context window for the model that owns this entry.',
  imcUtilization: 'The session\'s largest observed live context, including generated output, divided by the configured context window.',
