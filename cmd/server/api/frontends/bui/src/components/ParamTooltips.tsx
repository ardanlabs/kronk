...
export const PARAM_TOOLTIPS = {
  ...
  nthreads: "Number of CPU threads used for single-token generation. More threads can speed up CPU-bound work but may cause contention on busy systems. 0 or empty = the greater of Yzma's hardware-aware default and the logical CPU count.",
  nthreadsBatch: "Number of CPU threads used for prompt processing and other multi-token batches. 0 or empty = the greater of Yzma's hardware-aware default and the logical CPU count.",
  ...
  projDevice: 'Backend device used by the multimodal projector (mmproj), such as CUDA1 or MTL0. Automatic lets the runtime choose. Cannot be combined with proj-on-cpu enabled.',
  ...
  mtpNDraft: 'Fixed number of draft tokens per round for the auto-detected MTP head baked into this model. Leave empty to use the default of 3.',
  ...
  imcSystemSnapshotBytes: 'RAM occupied by the target snapshot, compatible draft snapshot, and saved MTP hidden row.',
  imcSystemRestores: 'Number of working caches preloaded from this System cache entry.',
  imcSystemActiveRestores: 'Number of requests currently restoring this entry. An entry in use cannot be replaced.',
  imcPeakContext: 'All-time largest live context observed for this cache entry across every request, including input and generated output. It may come from an earlier request.',
  imcMessages: 'Number of complete messages represented by the current working cache.',
  imcInputMessages: 'Number of messages in the latest completed request. The current snapshot is published during prompt setup, so it can temporarily contain more messages while a newer request is generating.',
  imcInputTokens: 'Complete input tokens in the latest completed request, including the stable conversation render and generation-template tail. Current tokens can temporarily be larger while a newer request is generating.',
  imcOutputTokens: 'Reasoning and completion tokens generated by the latest completed request.',
  imcRequestTotal: 'Total live context reached by the latest completed request: complete input tokens, including the generation-template tail, plus generated output tokens.',
