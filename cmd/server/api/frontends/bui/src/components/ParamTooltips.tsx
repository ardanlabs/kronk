...
export const PARAM_TOOLTIPS = {
  ...
  imcAllocated: 'Largest physical context retained by the current working cache. It does not include the fallback cache allocation.',
  imcCheckpointContext: 'Physical context retained by the fallback cache. Kronk can restore this checkpoint when later template output no longer matches the current snapshot.',
  imcCheckpointAllocated: 'Largest physical context retained by the fallback cache backing allocation.',
  ...
  imcFallbackTokens: 'Tokens represented by the fallback cache. Kronk can restore this state when a newly rendered prompt no longer matches the current working cache, such as when a template removes prior-turn reasoning.',
  imcFallbackKind: 'Fallback cache state. User is a verified user-message boundary; calculating means Kronk is calculating the next current cache or the fallback is not yet verified as a user boundary.',
  imcFallbackUpdates: 'Number of times Kronk installed or advanced this entry’s fallback cache.',
