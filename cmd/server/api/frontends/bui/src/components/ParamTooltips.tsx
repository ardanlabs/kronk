...
export const PARAM_TOOLTIPS = {
  ...
  imcContext: 'Physical KV-cache cells currently stored in the rolling snapshot. For text-only sessions this is the number of stored token positions.',
  ...
  imcInputMessages: 'Number of messages in the latest request completed by this session.',
  imcInputTokens: 'Complete input tokens in the latest request, including the stable conversation render and generation-template tail.',
  imcOutputTokens: 'Reasoning and completion tokens generated by the latest request completed by this session.',
  imcReusableMessages: 'Complete message count represented by the alternate reusable snapshot. A dash means the reusable state ends at an exact token boundary that cannot be mapped safely to a message count.',
  imcReusableTokens: 'Tokens in the alternate reusable snapshot. Kronk can restore these when the complete current input snapshot diverges after this boundary.',
  imcRequestTotal: 'Latest request input tokens plus generated output tokens.',
  imcRequestUtilization: 'Latest request input plus output tokens divided by the configured context window.',
