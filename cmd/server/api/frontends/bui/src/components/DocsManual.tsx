...
kronk bucky model remove tiny`}</code></pre>
  ...
  -F response_format=json`}</code></pre>
          ...
          <p>When every state is in use, another call waits until a state becomes available or its context is canceled. Bucky has no separate queue-depth or admission-timeout setting. A stream reserves one state for its lifetime, so always close streams that are no longer needed.</p>
...
.manual chapter Markdown    -> DocsManual.tsx`}</code></pre>
          ...
          <p>The handle semaphore and state pool are both sized by <code>Config.NSeqMax</code>, which defaults to 1. Calls beyond that capacity wait until their context is canceled or an operation releases its state; Bucky has no queue-depth or admission-timeout setting. The states isolate concurrent inference while sharing the handle's model weights and Whisper context.</p>
