...
make benchmark-rerank-batchseq`}</code></pre>
          ...
          <p>One fallback heuristic is based on a common user-turn transition. Kronk does not assume that existing user-message content changes. It assumes that adding a new real user message may cause the chat template to re-render earlier assistant messages differently, especially by removing or relocating reasoning content from a prior assistant turn. Kronk therefore retains the complete stable state at a real-user boundary before extending the rolling state through assistant and tool activity. If a later user turn makes the rolling state incompatible, this earlier checkpoint may still prefix the new rendering.</p>
          <p>This is a checkpoint-placement heuristic, not the cache-matching rule. The planner always compares complete rendered token or media plans, restores the longest compatible rolling or fallback state, and rebuilds when neither is a safe prefix. A prefix-stable template can continue using the longer rolling state without touching the fallback checkpoint.</p>
...
Actual plan: [A B C D][G]`}</code></pre>
          ...
          <p>Kronk searches available sessions for the longest complete saved plan that is a prefix of the new stable plan. A text session always has its latest rolling snapshot and can retain at most one additional alternate reusable snapshot captured at a previously verified token boundary. This is one rolling snapshot plus one alternate, not two alternate checkpoints:</p>
...
Reusable:   [A B C]       -> also safe if its rolling snapshot diverges`}</code></pre>
...
Actual:        [A B C D E F][G]`}</code></pre>
          ...
          <p>If the rolling snapshot diverges because a template retroactively changes historical rendering, append can instead begin from the alternate reusable snapshot. Kronk restores that complete state and prefills everything after it; it never rewinds a later model state or deletes an arbitrary KV range.</p>
...
exact read-only hit or media-anchor advance:
    keep reservation -> generate -> release at completion`}</code></pre>
          ...
          <p>Kronk compares the complete stable token sequence with the rolling and alternate reusable sequences in existing sessions. The result is one of three match types:</p>
          <ul>
...
data: [DONE]`}</code></pre>
          ...
          <p>If generation reaches the output-token limit while native tool output is buffered, Kronk discards the entire tool round. Recognized complete and truncated native tool-call syntax is omitted from assistant content, and no executable tool calls are returned from that round. Ordinary assistant text outside recognized tool spans is preserved when the parser can safely distinguish it. The terminal response keeps <code>finish_reason: "length"</code>, and usage still includes every generated token. An earlier streaming activity delta may contain the function name; consumers must wait for completed arguments and terminal <code>finish_reason: "tool_calls"</code> before execution.</p>
...
Then one line per non-confirmed flag: \`Dismissed: <probe-id> — <reason>\`.`}</code></pre>
  ...
  -> protocol response`}</code></pre>
          ...
          <p>A text session currently retains the latest rolling snapshot and at most one additional alternate reusable snapshot. The alternate can begin as a complete user-turn checkpoint and later become a progressive token-boundary checkpoint when Kronk restores an earlier state, recomputes to a longer exact common prefix, and snapshots that prefix. Publishing the progressive checkpoint replaces the prior alternate; Kronk does not also retain the older user-turn state. Thus, "one alternate snapshot" can mean two complete model states are allocated for the session—rolling plus alternate—but it never means two alternate checkpoints. This is a deliberate host-memory tradeoff, not proof that the older boundary is unimportant.</p>
          ...
          <p>An observed Gemma 4 MTP trace exposed a different pattern. The template's look-ahead continuation logic changed a previously rendered assistant/tool turn ending after later assistant messages were appended. Several tool-loop iterations diverged two tokens before the prior rolling boundary: 21,212 became a 21,210-token common prefix, 23,105 became 23,103, and later turns showed the same pattern. Progressive checkpoints sped up those intra-turn recoveries, but they displaced the older 11,799-token complete-user turn checkpoint. When a new user message subsequently removed prior-turn reasoning, the prompt matched through approximately 11,802 tokens but Kronk could no longer restore the required 11,799-token state, so it rebuilt another cache entry.</p>
