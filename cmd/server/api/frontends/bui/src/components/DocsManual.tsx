...
models:
  ...
  nseq-max: 4`}</code></pre>
          ...
          <p>Hybrid describes a separate design choice: which sequence-processing mechanism each layer uses. For example, Qwen 3.6 combines Gated DeltaNet and full-attention layers while using an MoE feed-forward block in every layer. Every token still traverses every layer; the hybrid stack changes how context is processed, and the MoE router changes which feed-forward experts activate.</p>
