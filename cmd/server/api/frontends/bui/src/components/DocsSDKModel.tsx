  ...
  return (
    <div>
      <div className="page-header">
        <h2>Model Package</h2>
        <p>Package model provides the low-level api for working with models. This is an internal package and should not be used directly. If you use this package directly you are on your own. The project reserves the right to break this API.</p>
      </div>
        ...
        <div className="doc-content">
          ...
          <div className="card" id="functions">
            ...
            <div className="doc-section" id="func-parsesplitmode">
              ...
              <p className="doc-description">ParseSplitMode parses a string into a SplitMode. Supported values are "none", "layer", "row", and "tensor".</p>
            </div>
          </div>
	...
	// Has unexported fields.
              ...
              <p className="doc-description">Config represents model level configuration. These values if configured incorrectly can cause the system to panic. The defaults are used when these values are set to 0.</p>
              <p className="doc-description">Adapters contains local llama.cpp-compatible LoRA adapter GGUF files to load with the model. Adapter scales are fixed for the lifetime of the model.</p>
              <p className="doc-description">ArtifactIntegrity contains expected artifact verification state keyed by model file path. It is used to avoid repeating completed verification work.</p>
              <p className="doc-description">AdmissionTimeout limits how long a request waits for an admission permit. The timeout applies only to admission, not request processing after a permit is acquired. When unset or set to 0, the default is 3 minutes.</p>
              <p className="doc-description">AutoTune, when true, asks kronk.New to run a hardware-aware analysis of the model (architecture, size, and available devices) and seed unset settings (context window, KV cache type, slots, flash attention, split mode, etc.) before loading. It is off by default for backwards compatibility, and any option the caller sets explicitly always wins over the analysis. It has no effect when using the low-level model package directly (only kronk.New applies it).</p>
              <p className="doc-description">AutoTuned records that an upstream owner such as the Kronk Model Server has already applied AutoTune. When AutoTune and AutoTuned are both true, kronk.New preserves the enabled state for diagnostics without repeating the hardware analysis.</p>
              <p className="doc-description">CacheMinTokens sets the minimum token count required before caching. Messages shorter than this threshold are not cached, as the overhead of cache management may outweigh the prefill savings. When set to 0, defaults to 100 tokens.</p>
              <p className="doc-description">CacheTypeK is the data type for the K (key) cache. This controls the precision of the key vectors in the KV cache. Lower precision types (like Q8_0 or Q4_0) reduce memory usage but may slightly affect quality. When left as the zero value (GGMLTypeAuto), the default llama.cpp value is used.</p>
              ...
              <p className="doc-description">PtrFlashAttention controls Flash Attention mode. Flash Attention reduces memory usage and speeds up attention computation, especially for large context windows. When nil, FlashAttentionAuto is used so llama.cpp can decide whether the active backend supports it. Set to FlashAttentionEnabled to force it on, or FlashAttentionDisabled to force it off.</p>
              <p className="doc-description">IMCSessionCapacity sets the number of reusable IMC session identities. When left unset or set to 0, generation models default to NSeqMax * QueueDepth. An explicit value must be at least that large so every admitted generation request can reserve a session.</p>
              ...
              <p className="doc-description">PtrMoE controls expert-tensor placement for Mixture of Experts models.</p>
              ...
              <p className="doc-description">RopeScaling controls the RoPE scaling method for extended context support. Set to RopeScalingYaRN only when the model supports YaRN and configure the frequency scale required by that model.</p>
              <p className="doc-description">SessionStoreFactory constructs session stores for direct SDK use. Kronk invokes it separately for every working session store it needs and closes every successfully returned store. The factory must return a new, independent store on each call. The factory may capture shared, concurrency-safe resources such as a connection pool or process-wide quota. It receives the context for the operation that needs the store. Store read, prepare, commit, and reset operations also receive that context and return errors. System prompt preloads always remain in RAM. When nil, Kronk uses the built-in RAM factory. Backend-specific constructor parameters belong to the backend package and are captured by the injected factory.</p>
              <p className="doc-description">SplitMode controls how the model is split across multiple GPUs: - SplitModeNone (0): single GPU - SplitModeLayer (1): split layers and KV across GPUs - SplitModeRow (2): legacy row-split parallelism - SplitModeTensor (3): tensor parallelism</p>
              <p className="doc-description">When nil (not set), the default is SplitModeLayer, matching llama.cpp. Layer mode distributes a single GGUF across multiple GPUs without requiring the backend-specific support used by row and tensor modes.</p>
              <p className="doc-description">SWAFull controls whether models with sliding window attention (SWA) use a full-size KV cache for SWA layers instead of the memory-efficient small cache. When nil (default), llama.cpp's default is used. When explicitly set to false, SWA layers only cache the last n_swa tokens, saving significant VRAM but limiting context caching and shifting. When true, SWA layers use the full context window for their KV cache, preserving accuracy at the cost of higher memory usage.</p>
              <p className="doc-description">TensorBuftOverrides is a list of tensor buffer type override patterns that force matching tensors to execute on CPU instead of GPU. This is an expert-level configuration useful for MoE models where certain FFN expert tensors don't fit in VRAM. Supported values: - "all-ffn": offload all FFN expression tensors to CPU - "block:N": offload FFN tensors for block N to CPU (e.g., "block:12") - Any regex pattern matching tensor names (e.g., `blk\.12\.ffn_(up|down|gate)`)</p>
              <p className="doc-description">TensorSplit controls how model layers are proportionally distributed across multiple GPUs. Each element represents the fraction of the model assigned to the corresponding device. For example, [0.6, 0.4] splits 60%/40% across two GPUs. The length must match the number of devices. When empty, the split is determined automatically based on available VRAM.</p>
              <p className="doc-description">YarnAttnFactor sets the YaRN attention magnitude scaling factor. When nil, uses the model or llama.cpp default.</p>
              <p className="doc-description">YarnBetaFast sets the YaRN low correction dimension. When nil, uses the model or llama.cpp default.</p>
              <p className="doc-description">YarnBetaSlow sets the YaRN high correction dimension. When nil, uses the model or llama.cpp default.</p>
              <p className="doc-description">YarnExtFactor sets the YaRN extrapolation mix factor. When nil, uses the model or llama.cpp default. Set to 0 to disable extrapolation.</p>
              <p className="doc-description">YarnOrigCtx sets the original training context size for YaRN scaling. When nil or 0, uses the model's native training context length from metadata.</p>
            </div>
	...
	TensorSplit   []float32 // Per-device tensor split for draft model
              ...
              <p className="doc-description">DraftModelConfig configures speculative decoding for a target model. It serves two purposes depending on whether ModelFiles is set:</p>
              <p className="doc-description">1. Separate-GGUF draft (ModelFiles set): a smaller, faster model generates candidate tokens that the target verifies in a single forward pass. Requires NSeqMax == 1 (single-slot mode) and a draft that shares the target's vocabulary (same tokenizer).</p>
              <p className="doc-description">2. MTP nDraft override (ModelFiles empty): when the target GGUF ships an auto-detected MTP head, this block sets the number of draft tokens per round without supplying a separate model. NDraft defaults to defMTPNDraft when left unset.</p>
              <p className="doc-description">A model can have at most one drafter. If ModelFiles is set, the separate-GGUF drafter wins even on a target that also has an MTP head.</p>
            </div>
	...
	Content string
              ...
              <p className="doc-description">Result is the per-token outcome returned by StateMachine.Classify.</p>
              <p className="doc-description">Content may be empty when the token is a structural marker that has been fully consumed by the state machine (e.g. &lt;think&gt;, &lt;tool_call&gt;). When Content is non-empty, it is routed to the appropriate accumulator based on Channel.</p>
            </div>
	...
	Flush() Result
              ...
              <p className="doc-description">StateMachineFlusher is implemented by state machines that may retain model output not yet returned by Classify.</p>
              <p className="doc-description">Flush drains that output at successful end-of-generation. It must not return content previously returned by Classify. Callers invoke Flush until it returns a zero Result so state machines can preserve multiple channel transitions retained from one decoded piece.</p>
            </div>
	...
	ConsumeVocabEOG(content string)
          ...
          <div className="card" id="methods">
            ...
            <div className="doc-section" id="method-model-embeddings">
              ...
              <p className="doc-description">Embeddings performs embedding for one or more inputs.</p>
              <p className="doc-description">Supported options in d: - input ([]string): the texts to embed (required) - truncate (bool): if true, truncate inputs to fit context window (default: false) - truncate_direction (string): "right" (default) or "left" - dimensions (int): reduce output to first N dimensions (for Matryoshka models)</p>
              <p className="doc-description">Supported models process inputs together as a multi-sequence batch. Other models use the context-pool fallback.</p>
            </div>
            ...
            <div className="doc-section" id="method-model-rerank">
              ...
              <p className="doc-description">Rerank performs reranking for a query against multiple documents. It scores each document's relevance to the query and returns results sorted by relevance score (highest first).</p>
              <p className="doc-description">Supported options in d: - query (string): the query to rank documents against (required) - documents ([]string): the documents to rank (required) - top_n (int): return only the top N results (optional, default: all) - return_documents (bool): include document text in results (default: false)</p>
              <p className="doc-description">Supported models process documents together as a multi-sequence batch. Other models use the context-pool fallback.</p>
            </div>
            ...
            <div className="doc-section" id="method-model-tokenize">
              ...
              <p className="doc-description">Tokenize returns the token count for a text input.</p>
              <p className="doc-description">Supported options in d: - input (string): the text to tokenize (required) - apply_template (bool): if true, wrap input as a user message and apply the model's chat template before tokenizing (default: false) - add_generation_prompt (bool): when apply_template is true, controls whether the assistant role prefix is appended to the prompt (default: true)</p>
              <p className="doc-description">When apply_template is true, the returned count includes all template overhead (role markers, separators, generation prompt). This reflects the actual number of tokens that would be fed to the model.</p>
            </div>
	...
	SpeculationMTP      = internalspec.ModeMTP
            ...
            <div className="doc-section" id="const-defaultqueuedepth">
              ...
              <p className="doc-description">DefaultQueueDepth is the number of admitted request layers used when queue depth is not explicitly configured.</p>
            </div>
